...
The coordinated patterns deliberately vary their color treatment: Power-Up and Blackout Reveal use full-sign rainbows; Typewriter uses one consistent hue for an entire word and advances to a different hue on its next run; Signal Relay and Letter Domino rotate colors as they travel; Comet Letters assigns a different rainbow hue to each letter; and Protocol Gold keeps its gold/white signature. Fade Out preserves whichever colors are already showing as it takes the sign to black.
