...
void renderColorDropPattern(int colorIndex) {
  constexpr int LAST_FLAG_LED = NUM_F - 1;
  static int pos1 = LAST_FLAG_LED;
  ...
  if (pos1 < LAST_FLAG_LED) Strip_F[pos1 + 1] = color1;
  --pos1;
  if (pos1 < 0) {
    pos1 = LAST_FLAG_LED;
    Strip_F[0] = color1;
  ...
  if (pos2 < LAST_FLAG_LED) Strip_F[pos2] = CRGB::White;
  if (pos2 < LAST_FLAG_LED - 1) Strip_F[pos2 + 1] = color2;
  ...
  if (pos3 < LAST_FLAG_LED) Strip_F[pos3] = CRGB::White;
  if (pos3 < LAST_FLAG_LED - 1) Strip_F[pos3 + 1] = color3;
}
